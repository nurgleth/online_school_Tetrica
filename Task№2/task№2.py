"""
В нашей школе мы не модем разглашать персональные данные пользователей, но чтобы преподаватель и ученик смогли
...
HEADERS = {
    "uesr-agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) "
...
def parse_count_page(html):
    ...
    count_page = soup.find("div", id="mw-pages").find("p").get_text()  # показывает количесвто страниц
...
def get_content(html):
    ...
    if URL == link_next_page:
        ...
        link_next_page2 = HOST + str_link_next_page.get("href")  # для первой странице это пустая пследовательность
        URL[0] = link_next_page2
